...
void ERSimtoRaw::Exec(Option_t* opt)
{
  ...
  for (Int_t iSignal = 0; iSignal < fPixelSignals->GetEntriesFast(); iSignal++){
    ...
    // if (signal->GetSide() !=1 || signal->PixelNb() != 25) continue;
    if ( signal->GetPixelNb() != 136 ) continue;
...
//        cout << " not NULL event found " << endl;
  ...
  for (Int_t nPoint = 0; nPoint< fNPoints; nPoint++)
  {
    if (nPoint<=StartTime*10 || nPoint>=(FinishTime*10-1)) {
      if (signal->GetSide() == 0) {fRawEvents[0]->SetAmp(0, nPoint);}
      if (signal->GetSide() == 1) {fRawEvents[1]->SetAmp(0, nPoint);}
    } else {
       if (signal->GetSide() == 0) {fRawEvents[0]->SetAmp(-result->GetAt(nPoint - StartTime*10), nPoint);} //fill the RawEvent
       if (signal->GetSide() == 1) {fRawEvents[1]->SetAmp(-result->GetAt(nPoint - StartTime*10), nPoint);}
      // fRawEvents[0]->SetAmp(0, nPoint); //fill the RawEvent
    }
    
    if (signal->GetSide() == 0) {fRawEvents[0]->SetTime(nPoint*0.1, nPoint);}
    if (signal->GetSide() == 1) {fRawEvents[1]->SetTime(nPoint*0.1, nPoint);}
  }

 	if (signal->GetSide() == 0) fRawEvents[0]->SetStartTime(StartTime);
  if (signal->GetSide() == 1) fRawEvents[1]->SetStartTime(StartTime);
  if (signal->GetSide() == 0) fRawEvents[0]->SetFinishTime(FinishTime);
  if (signal->GetSide() == 1) fRawEvents[1]->SetFinishTime(FinishTime);
