...
Int_t ERQTelescopeGeoPseudoDoubleSi::fConstructedObjCount = 0;
//--------------------------------------------------------------------------------------------------
ERQTelescopeGeoPseudoDoubleSi::ERQTelescopeGeoPseudoDoubleSi() {
}
//--------------------------------------------------------------------------------------------------
ERQTelescopeGeoPseudoDoubleSi::ERQTelescopeGeoPseudoDoubleSi(TString typeFromXML1, 
                                                             TString id1, 
                                                             TString typeFromXML2, 
                                                             TString id2,
...
void ERQTelescopeGeoPseudoDoubleSi::ConstructGeometryVolume(void) {
  ParseXmlParameters();
  // --------------------------------------------------------------------------
  ...
  cout << "ERQTelescopeGeoPseudoDoubleSi::this->GetVolumeName() " << this->GetVolumeName() << endl;
  ...
  Double_t boxX = fSensX / fStripCountX;  
  Double_t boxY = fSensY / fStripCountY; 
  cout << "ERQTelescopeGeoPseudoDoubleSi::boxX " << boxX << endl;
  cout << "ERQTelescopeGeoPseudoDoubleSi::boxY " << boxY << endl;
  // Double_t boxZ = stripZ; 
  // read thikness maps
  auto file = TFile::Open(fPathInhomogenMap.Data());
...
// TGeoTranslation *t2 = new TGeoTranslation("t2",0,0, 20);
  ...
  for (Int_t iStripX = 0; iStripX < fStripCountX; iStripX++) {
    ...
    for (Int_t iStripY = 0; iStripY < fStripCountY; iStripY++) {
      ...
      cout << "ERQTelescopeGeoPseudoDoubleSi::sensThckss " << sensThckss << endl;
  ...
  for (Int_t iStripX = 0; iStripX < fStripCountX; iStripX++) {
    for (Int_t iStripY = 0; iStripY < fStripCountY; iStripY++) {
      ...
      Double_t deadFrontThckss = histFrontDead->GetBinContent(iStripX + 1, iStripY + 1);
      Double_t deadBackThckss = histBackDead->GetBinContent(iStripX + 1, iStripY + 1);
      Int_t boxNb = iStripX * fStripCountY + iStripY;
...
//--------------------------------------------------------------------------------------------------
void ERQTelescopeGeoPseudoDoubleSi::ParseXmlParameters() {
  ...
  cout << "fComponentId1 " << fComponentId1 << endl;
  cout << "fComponentId2 " << fComponentId2 << endl;
