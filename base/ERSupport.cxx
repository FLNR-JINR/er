...
//  while (curStep < range) {
//    Double_t eloss = calc->ComputeDEDX(T, &ion, "hIoni", &mat) * tempStep * 10 /*cm to mm*/;
//    integralEloss += eloss;
//    T += eloss;
//    curStep += tempStep;
//  }
...
Double_t CalcElossIntegralVolStep (Double_t kineticEnergy, const G4ParticleDefinition& particle, 
                                   const G4Material& material, const Double_t range, const Double_t /*intStep=1e-4*/) {
  if (range <= 0.)
    return 0;
  Double_t integralEloss = 0.;
...
Double_t ComputeElossIntegralVolStep(Double_t kineticEnergy, const G4ParticleDefinition& particle, 
                                     const G4Material& material, const TString& process_name,
  ...
  G4EmCalculator* calc = new G4EmCalculator();
  while (curStep < range) {
   Double_t eloss = calc->ComputeDEDX(kineticEnergy, &particle,
                                      process_name.Data(), &material) * intStep * 10 ; // MeV
    integralEloss += eloss;
    kineticEnergy += eloss;
    curStep += intStep;
  }
