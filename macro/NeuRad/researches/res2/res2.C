...
void res2(int eKin, TString pngdir, TString pngname, int pe, int sumpe, Double_t thEdep,TString fsimName, TString fdigiName){
  ...
  Double_t sumFiberThreshold = sumpe*0.5*(0.1 + 1.3)*5.;
  
  Long_t nEvent = T->GetEntries();
  
  TString title;
  title.Form("XY distance, neutron, Ekin=%d MeV, %d events, pixel threshold %d pe, fiber threshold %d pe", eKin,nEvent, pe, sumpe);
  ...
  title.Form("XY distance less 6mm, neutron, Ekin=%d MeV, %d events, pixel threshold %d pe, fiber threshold %d pe",eKin,nEvent, pe, sumpe);
  ...
  for (int iEvent = 0; iEvent < nEvent; iEvent++){
    ...
    for (int i = 0; i<64; i++){
      if (fb[i] >1){//Загорелось хотя бы с одной стороны
        ...
        /*
        for (int iLink = 0; iLink < sig1->GetNLinks();iLink++){
          link = sig1->GetLink(iLink);
          ERNeuRadFiberPoint* fpoint = (ERNeuRadFiberPoint*)fpoints->At(link.GetIndex());
          modA += fpoint->PhotoElCount();
        }
        
        for (int iLink = 0; iLink < sig2->GetNLinks();iLink++){
          link = sig2->GetLink(iLink);
          ERNeuRadFiberPoint* fpoint = (ERNeuRadFiberPoint*)fpoints->At(link.GetIndex());
          modA += fpoint->PhotoElCount();
        }
        */
        if ((sig1->GetFirstInteg(50.) + sig2->GetFirstInteg(50.)) > sumFiberThreshold){
  ...
  title.Form("%s/%d_MeV_distXY_%s_%dev_pth_%dpe_fth_%dpe.png",pngdir.Data(),eKin,pngname.Data(), nEvent, pe, sumpe);
  c1->SaveAs(title.Data());
  
  c1->Clear();
  hdxyLess6->Draw();
  gSystem->ProcessEvents();
  title.Form("%s/%d_MeV_distXY_less6_%s_%dev_pth_%dpe_fth_%dpe.png",pngdir.Data(),eKin,pngname.Data(),nEvent, pe, sumpe);
  c1->SaveAs(title.Data());
}