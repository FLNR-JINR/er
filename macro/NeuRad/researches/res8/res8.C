/*
"однофайберный" режим в геометрии v2.
*/

void res8(int eKin, TString pngdir, TString pngname, int pe, Double_t thEdep,TString fsimName, TString fdigiName){
  ...
  for (int iEvent = 0; iEvent < nEvent; iEvent++){
    ...
    for (int iSignal = 0; iSignal < sigCount; iSignal++){
        ...
        //cout << bundle << " " << fiber << endl;
        if (signal->GetFirstInteg(100.) > fiberThreshold){
          fb[bundle][fiber]++;
          int nSig = jsig[bundle][fiber]++;
          signals[bundle][fiber][nSig] = iSignal;
        }    
   }
