...
sensor x32 different types - sector (1) x32 same - dead zone (1) - RTelescope(1,2) - RTelescopes - top
sector (2) x32 same - dead zone (2) - RTelescope(1,2) - RTelescopes - top
crystal - crystalArray x16 same - RTelescope(1,2) - RTelescopes - top
...
void create_RTelescope_geo_v3()
{
    ...
    // Positioning sectors inside the dead ring
    TString curRotName;
    ...
    TGeoVolume* CsIarrayR1 = gGeoMan->MakeTubs("crystalArrayR1", pAir, x-0.1, (x+t1_dz+t2_dz)+0.1, 1.9/2., 0., 360.);
    TGeoVolume* CsIarrayR2 = gGeoMan->MakeTubs("crystalArrayR2", pAir, x-0.1, (x+t1_dz+t2_dz)+0.1, 1.9/2., 0., 360.);
