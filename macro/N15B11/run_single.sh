...
cd -
echo -e "\e[1m\e[32m========== Cleanup finished  === Angle( ${ANGLE} ) ============ \e[0m"

if [ -d geo/ ]; then
    cd geo/
    root -l -b -q "create_N15B11_detector_geo.C(${ANGLE})"
    wait
    cd -
else
...
echo -e "\e[1m\e[32m========== Geometry created  === Angle( ${ANGLE} ) ============ \e[0m"

root -l -b -q "sim.C(${NEVENTS}, 0, \"${OUTDIR}\", ${ANGLE})" > ${OUTDIR}/out.txt 2> ${OUTDIR}/err.txt
wait
